(() => {
  let entry = data.header.subtitle[Math.floor(Math.random() * data.header.subtitle.length)];
  if (typeof entry == "string"){document.getElementById("headerSubtitle").innerHTML = entry;}
  if (typeof entry == "function"){entry();}
  if(entry === "There are currently quotes (including this one) on the site. Reload to see if your submission was one of them!"){document.getElementById("headerSubtitle").innerHTML = `There are currently ${data.header.subtitle.length} quotes (including this one) on the site. Reload to see if your submission was one of them!`;}
