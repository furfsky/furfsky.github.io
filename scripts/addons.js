(() => {
  ...
  data.addons.forEach((addon) => {
    ...
    if (addon.title)
      text.innerHTML += ` <span class="addonTitle">[${
        addon.shortSuffix ? addon.shortSuffix : `!`
      }]<span>${addon.title}</span></span>`;
    if (addon.fileLink)
      addonButton.innerHTML = `<a href="${addon.fileLink.toString()}" download>${
        addon.shorterName
      }</a>`;
    if (addon.fileLinkTwo)
      addonButtonTwo.innerHTML = `<a href="${addon.fileLinkTwo.toString()}" download>${
        addon.shorterNameTwo
      }</a>`;
